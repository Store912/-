...
function addGiftToCart(productId, productName, normalPrice, image = null) {
    ...
    const existingItem = cart.find(item => item.id === productId && item.isGift === isEligibleForGift);
    if (existingItem) {
        // Si ya es regalo, no permitir más de 1 como regalo
        if (existingItem.isGift && isEligibleForGift) {
            showNotification('Solo puedes elegir 1 regalo gratis.');
            return;
        } else {
            existingItem.quantity += 1;
        }
...
function updateQuantity(productId, change) {
    ...
    if (item) {
        // Si es regalo y se intenta aumentar la cantidad, no hacer nada
    ...
    if (computedDiscount > 0) {
        html += `
            <div class="payment-item" style="margin-top:6px;">
                ...
                <span >-$${computedDiscount.toLocaleString()} COP</span>
            </div>
        `;
    }
